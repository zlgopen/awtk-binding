 ...
 */
 ...
 public  TRet fillText(String text, double x, double y, double max_width)  {
   ...
   * @return 返回RET_OK表示成功，否则表示失败。
   */
 public  TRet fillTextByGlyphs(long glyphs, int start, int len, int x, int y, double max_width)  {
   return TRet.from(vgcanvas_fill_text_by_glyphs(this != null ? (this.nativeObj) : 0, glyphs, start, len, x, y, max_width));
 }
...
static private native int vgcanvas_fill_text_by_glyphs(long vg, long glyphs, int start, int len, int x, int y, double max_width);
